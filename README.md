...
The loss function is continously evaluated against using this board as input nodes and output nodes are either winner move O or blocker move X at position 1(column),2(row):
...
The assert is to play draw against tree-search player, with neural net starting. And the result is that the test is almost never successful. It seems to be between 5 and 10% of training attempts that is able to play draw against tree-search. No clear pattern can be seen in loss function plots for the successful tests. 
